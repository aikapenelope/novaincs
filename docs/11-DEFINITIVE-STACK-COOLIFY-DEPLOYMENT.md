...
Nova runs on a single Hetzner CX43 VPS (8 vCPU, 16 GB RAM, 160 GB NVMe) in Helsinki, Finland. The catalog PWA runs on Cloudflare Workers at the edge. Images are stored on Cloudflare R2.
...
Dokploy manages deployment on the CX43. Git-based auto-deploy via Traefik for SSL/routing. The catalog deploys separately to Cloudflare Workers via `wrangler deploy`.
...
|---|---|---|
| 0-3,000 merchants | 1x CX43 (Helsinki) | ~€65/mo |
| 3,000-8,000 | 1x CX53 (16 vCPU, 32 GB) | ~€80/mo |
